...
def data():
    ...
    while True:
        ...
        if any(status in user.strip() for status in valid_status) and user.strip() not in servers:
            servers.append(user)
        else: 
            print("Please enter a valid server name and statut")
...
def write(online,offline,manage):
		...
		if online:
		    with open("online.txt","a") as f:
		        f.write("\n".join(online)+"-"+timestamp+"\n")
		if offline:
		    with open("offline.txt","a") as f:
		        f.write("\n".join(offline)+"-"+timestamp+"\n")
		if manage:
		    with open("manage.txt","a") as f:
		    	f.write("\n".join(manage)+"-" +timestamp+"\n") 
	    		
#Summary

def summary(online,offline,manage):
		return f"\n=======SERVER STATUS REPORT========	\nOnline:  \t{len(online)} server(s)\nOffline: \t {len(offline)} server(s)\nManage: \t {len(manage)} server(s)\n=====================\nServers written respectively to online.txt,offline.txt, manage.txt\n"
